...
subprocess.run(['patch','-p1','-i','/port/kernel/patches-experimental/usb-otg-core-reinit.patch'],cwd=base,check=True)
...
static int core_exits,core_inits,core_error,core_unbind,phy_refs;
#define DWC3_GCTL_PRTCAP_DEVICE 2
static void dwc3_core_exit(struct dwc3 *d) {
 (void)d; assert(mutex && !spin); assert(phy_refs==1); phy_refs--; core_exits++;
}
static int dwc3_core_init(struct dwc3 *d) {
 assert(mutex && !spin); assert(phy_refs==0); core_inits++;
 if(core_unbind) d->gadget_driver=NULL;
 if(core_error) return core_error;
 phy_refs++; return 0;
}
static void dwc3_set_mode(struct dwc3 *d,int mode) {
 (void)d; assert(mutex && spin && phy_refs==1); assert(mode==DWC3_GCTL_PRTCAP_DEVICE);
}
...
static void reset(void) {
 ...
 core_exits=core_inits=core_error=core_unbind=0; phy_refs=1;
...
int main(void) {
 ...
 for(int i=1;i<=3;i++) {
  ...
  assert(starts==i && cleanups==i); assert(core_exits==i && core_inits==i && phy_refs==1);
 }
 ...
 for(int mode=0;mode<4;mode++) {
  ...
  assert(!dwc3_resume(&dev)); assert(!starts && !refs && !core_inits && !core_exits);
 }
 ...
 reset(); assert(!dwc3_resume_common(&d)); assert(!starts && !refs && !core_inits && !core_exits);
 ...
 /* Core error propagates before endpoints restart, with PM/locks balanced. */
 reset(); assert(!dwc3_suspend_common(&d)); core_error=-ETIMEDOUT;
 assert(dwc3_resume(&dev)==-ETIMEDOUT);
 assert(core_exits==1 && core_inits==1 && !starts && !refs && !spin && !mutex);
 /* Binding can disappear while the sleeping PHY operation runs. */
 reset(); assert(!dwc3_suspend_common(&d)); core_unbind=1;
 assert(!dwc3_resume(&dev)); assert(!starts && !refs && phy_refs==1);
